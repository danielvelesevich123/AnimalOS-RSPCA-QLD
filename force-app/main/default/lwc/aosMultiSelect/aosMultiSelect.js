...
import AosBase from 'c/aosBase';

export default class AosMultiSelect extends AosBase {
    ...
    validate() {
        ...
        this.template.querySelector('c-aos-select').clearErrors();
        if (!hasNoError) {
            this.template.querySelector('c-aos-select').showErrors(['Complete this field.']);
        }
        return hasNoError;
    }
    ...
    resetInput(event) {
        ...
        //     event.target.value = '';
        // } else {
            let select = this.template.querySelector('c-aos-select');
