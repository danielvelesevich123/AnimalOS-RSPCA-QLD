...
// Address mapping
let addressMappingCache = null;
const ADDRESS_MAPPING_STORAGE_KEY = 'animalos_address_mapping';
...
const loadAddressMapping = async () => {
    // Return cached value if available
    if (addressMappingCache !== null) {
        return addressMappingCache;
    }

    // Check localStorage
    try {
        const cachedData = localStorage.getItem(ADDRESS_MAPPING_STORAGE_KEY);
        if (cachedData) {
            addressMappingCache = JSON.parse(cachedData);
            return addressMappingCache;
        }
    } catch (e) {
        // localStorage not available or parse error, continue to load from Apex
    }

    // Load from static resource (unmanaged, dynamically loaded)
    try {
        // Fetch the actual resource
        const response = await fetch('/resource/AnimalOS_Address_Mapping_JSON');
        const mapping = await response.json();

        if (mapping) {
            addressMappingCache = mapping;
            // Store in localStorage
            try {
                localStorage.setItem(ADDRESS_MAPPING_STORAGE_KEY, JSON.stringify(addressMappingCache));
            } catch (e) {
        ...
        console.error('Error loading address mapping:', e);
    }

    return null;
};

// Load address mapping automatically on module initialization
loadAddressMapping();
...
const formatState = (stateValue) => {
    ...
    // Use cached mapping (loaded automatically on module init)
    if (!addressMappingCache) {
        return stateValue;
    }

    // Search states array for matching label or isoCode
    if (addressMappingCache.states && Array.isArray(addressMappingCache.states)) {
        for (const state of addressMappingCache.states) {
            if (stateValue.toLowerCase() === (state.label || '').toLowerCase() ||
                stateValue.toLowerCase() === (state.isoCode || '').toLowerCase()) {
                return state.isoCode || stateValue;
            }
        }
    }
...
const formatCountry = (countryValue) => {
    ...
    // Search countries array for matching label or isoCode
    if (addressMappingCache.countries && Array.isArray(addressMappingCache.countries)) {
        for (const country of addressMappingCache.countries) {
            if (countryValue.toLowerCase() === (country.label || '').toLowerCase() ||
                countryValue.toLowerCase() === (country.isoCode || '').toLowerCase()) {
                return country.isoCode || countryValue;
            }
        }
    }
